...
void RequestManager::handleFinished(QNetworkReply *networkReply)
{
    ...
    if (networkReply->error() == QNetworkReply::NoError)
    {
        ...
        {
            qDebug() << "Status: " << httpStatusCode << " Error!";
        }
...
/// \param code
///
void RequestManager::handleError(QNetworkReply::NetworkError code)
{
    qDebug() << "onError: " << code;
}
...
QNetworkRequest RequestManager::constructNetworkRequest(const QString hostName, QMap<QString, QString> headers)
{
    ...
    connect(&request, SIGNAL(onError(QNetworkReply::NetworkError)), this, SLOT(handleError(QNetworkReply::NetworkError)));
